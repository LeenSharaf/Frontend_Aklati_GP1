...
import 'package:roaa/Admin_cart/details.dart';
      ...
      builder: (context,provider,x) {
        return Scaffold(
          ...
          body: ListView.separated(
            ...
            scrollDirection: Axis.vertical,
            itemCount: provider.items.where((element) => element.discount>0).toList().length,
            itemBuilder: (context, index) {
              return InkWell(
                onTap: ()=>Navigator.of(context).push(MaterialPageRoute(builder: ((context) => Itm(shopdata: provider.items.where((element) => element.discount>0).toList()[index])))),
                child: Card(
                    child: ListTile(
                  leading:
                      Image.network(provider.items.where((element) => element.discount>0).toList()[index].img, height: 80, width: 80),
                  title: Row(
                    children: [
                      Text(
                        provider.items.where((element) => element.discount>0).toList()[index].name,
                        style: TextStyle(
                            fontSize: 15,
                            color: Colors.black,
                            fontFamily: "Mulish-VariableFont",
                  ...
                  subtitle: Text(
                    provider.items.where((element) => element.discount>0).toList()[index].discount.toString(),
                    style: TextStyle(
                        fontSize: 15,
                        color: Colors.black,
                        fontFamily: "Mulish-VariableFont",
                  ...
                  trailing: Text(provider.items.where((element) => element.discount>0).toList()[index].price.toString() + "\$",
                      style: TextStyle(
                          fontSize: 15,
                          color: Colors.black,
                          fontFamily: "Mulish-VariableFont",
